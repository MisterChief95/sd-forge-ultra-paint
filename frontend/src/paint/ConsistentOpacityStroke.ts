...
export class ConsistentOpacityStroke implements StrokeSession {
    ...
    private growForCurrentStamp(): boolean {
        ...
        const newTarget = createCompatibleRenderTexture(
            oldTarget,
            horizontal.size,
            vertical.size,
        );
        ...
        if (replacedTarget !== oldTarget) {
            ...
            newTarget.destroy(true);
            throw new Error(
                `[ultra-paint] failed to atomically grow raster layer "${this.layerId}"`,
            );
        }
    ...
    public end(_points: readonly StrokePoint[], cancelled: boolean): void {
        ...
        if (
            this.previewTexture &&
            this.store.getTexture(this.layerId) === this.target
        ) {
    ...
    private updateEraserPreview(
        setPreviewTexture: ((texture: Texture) => void) | undefined,
    ): void {
        if (
            !this.previewTexture ||
...
function calculateAxisGrowth(
    ...
    const desiredBefore = stampMin < 0
        ? Math.ceil(-stampMin + GROWTH_PADDING)
        : 0;
    const offset = Math.min(desiredBefore, available);
    const remaining = available - offset;
    const desiredAfter = stampMax > currentSize
        ? Math.ceil(stampMax - currentSize + GROWTH_PADDING)
        : 0;
